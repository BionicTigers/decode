...
class LimeLight(hardwareMap: HardwareMap, telemetry: Telemetry, val inInit: () -> Boolean, isRed: Boolean): System() {
    ...
    override val update = Command.continuous {
        result = limeLight.latestResult
        fiducials = result.fiducialResults

        val tag = if (isRed) fiducials.find { it.fiducialId == 24 } else fiducials.find { it.fiducialId == 20 }

    }

    fun readObelisk() = Command.create("read obelisk") {
        requires {
            return@requires inInit()
        }
        action {
